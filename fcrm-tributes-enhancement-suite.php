...
// Plugin constants
define("FCRM_ENHANCEMENT_VERSION", "1.4.0-beta.1");
...
class EnhancementSuite
{
	...
	{
		require_once FCRM_ENHANCEMENT_PATH .
			"includes/class-fcrm-enhancement-base.php";
		...
		require_once FCRM_ENHANCEMENT_PATH . "includes/class-fcrm-backup.php";
		require_once FCRM_ENHANCEMENT_PATH .
			"includes/class-fcrm-flower-delivery-disabler.php";
	}
	...
	private function setup_hooks(): void
	{
		...
		add_filter("plugin_action_links_" . FCRM_ENHANCEMENT_BASENAME, [
			$this,
			"add_plugin_links",
		]);
	}
	...
	public function requirement_notice(): void
	{
		?>
		 <div class="notice notice-error">
			 <p><?php _e(
    	"FirehawkCRM Tributes Enhancement Suite requires the FireHawkCRM Tributes plugin to be installed and activated.",
	...
	{
		$current_tab =
			isset($_GET["tab"]) && array_key_exists($_GET["tab"], $this->tabs)
    ...
    echo esc_html__(
    	"Originally developed for our internal use, this third-party plugin helps optimise and enhance your funeral website with the FireHawk Tributes plugin. It removes unnecessary scripts and styles to help improve site performance and includes colour styling UI options to help customise the tributes to better match your website styles. This plugin has no direct affiliation with FireHawkCRM or FireHawk Funerals.",
    	"fcrm-enhancement-suite"
    );
    echo "</p>";
    ?>
     	...
     	$tab_url = add_query_arg(
     		...
     		admin_url("admin.php")
					 ...
					 <a href="<?php echo esc_url($tab_url); ?>"
						 class="nav-tab <?php echo $current_tab === $tab_key
       	? "nav-tab-active"
       	: ""; ?>">
     ...
     endforeach; ?>
			 ...
			 <form method="post" action="options.php">
				 <?php // Only show the global save button on tabs where it's needed
     if ($current_tab === "optimisation") {
     	...
     	$this->modules[$current_tab]->render_settings();
			 ...
			 <?php // Add reset button for specific tabs
    if (method_exists($this->modules[$current_tab], "handle_reset")): ?>
				 <form method="post" style="margin-top: 20px;">
					 <?php wp_nonce_field("fcrm_reset_settings", "fcrm_reset_nonce"); ?>
					 <input type="hidden" name="module" value="<?php echo esc_attr(
      	$current_tab
      ); ?>">
					 <?php submit_button(
      	__("Reset Settings", "fcrm-enhancement-suite"),
      	"delete",
      	"reset",
      	false
      ); ?>
      	...
      	__(
      		'Need help or have a request? Contact our support team at %1$s or log an issue on %2$s.',
      		"fcrm-enhancement-suite"
      	),
