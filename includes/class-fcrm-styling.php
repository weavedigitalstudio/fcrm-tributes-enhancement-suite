...
class Styling extends Enhancement_Base
{
	...
	private $color_settings = [
		...
		"primary-button-hover-text" =>
			"Search &amp; Primary Button Hover Text Colour",
	...
	public function render_settings(): void
	{
		...
		<form method="post" action="options.php">
       	...
       	$this->get_option_prefix() . "grid-border-radius"
        	...
        	$this->get_option("grid-border-radius") ??
        		$this->get_default_value("grid-border-radius")
        ); ?>"
								class="regular-text"
							/>
							<p class="description">
								<?php echo esc_html__(
        	"Specify the border-radius for grid cards (e.g., 10px, 20px, 0).",
       	...
       	$this->get_option_prefix() . "border-radius"
						...
						<td>
							<input type="text"
								...
								value="<?php echo esc_attr(
        	$this->get_option("border-radius") ??
        		$this->get_default_value("border-radius")
        ); ?>"
								class="regular-text"
							/>
							<p class="description">
								<?php echo esc_html__(
        	"Specify the border-radius for buttons (e.g., 10px, 20px, 0).",
        	"fcrm-enhancement-suite"
        ); ?>
							</p>
						</td>
		...
		<div class="reset-defaults" style="margin-top: 20px;">
			<form method="post">
				...
				<?php submit_button(
    	__("Reset to Defaults", "fcrm-enhancement-suite"),
    	"secondary",
    	"reset-to-defaults",
    	false
    ); ?>
	...
	public function handle_reset(): void
	{
		if (
			...
			!wp_verify_nonce($_POST["fcrm_reset_nonce"], "fcrm_reset_to_defaults")
		) {
			return;
		}

		if (
			isset($_POST["fcrm_styling_reset_to_defaults"]) &&
			$_POST["fcrm_styling_reset_to_defaults"] === "true"
		) {
			// Reset color settings
			foreach ($this->color_settings as $key => $label) {
				update_option(
					$this->get_option_prefix() . $key,
					$this->get_default_value($key)
				);
			}
